...
import {toggleCommentLike as storeCommentLike} from "../../store/authSlice.js"
function LikeDislikeBtn({
            initialLikesOnComment=0 ,
            commentId
            }) 
    ...
    const handleLikeToggle = async () => {
        try {
          ...
          if (response) {
            ...
            // Update Redux store
            dispatch(storeCommentLike(commentId));
          }
        } catch (error) {
  ...
  return (
    ...
    <button onClick={handleLikeToggle} class="py-1.5 px-3 hover:text-green-500 hover:text-bold hover:scale-105 hover:shadow text-center border  rounded-md border-gray-400 h-8 text-sm flex items-center gap-1 lg:gap-2">
        <svg class="w-4 h-4 " xmlns="http://www.w3.org/2000/svg" fill={isAlreadyLiked?"green" : "none"} viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
